...
beta <- fread("/Users/bingbing/Desktop/noob_qcd_crossReactiveProbesRemoved.csv", data.table = F)
rownames(beta) <- beta$V1
beta <- beta[,-1]

## Load ref dataset shared in the pipeline package
load("/Users/bingbing/Desktop/EPIC_QC/IDOLOptimizedCpGs_REF.Rdata")

## Load the phenotype file
pheno <- read.csv("/Users/bingbing/Desktop/Pheno_QC.csv")

## Run the function
output <- estimate_cell_proportion(betavals = beta, phenotyps = pheno, cell_prop_ref = ref, xcol = "SampleID", ycol = 'row.names' )
write.csv(output, file = "/Users/bingbing/Desktop/Pheno_QC_1.csv", row.names = FALSE) # Save the phenotype file with cell types
